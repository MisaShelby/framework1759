...
import etu1759.framework.ModelView;
...
public class FrontServlet extends HttpServlet{
    ...
    public void getAllFile(String path) throws Exception{
        File modeleFolder = new File(path+"WEB-INF/classes/etu1759/Model/");
        String contents[] = modeleFolder.list();
        for (String file : contents) {
            file = file.replace(".class", "");
            Class<?> myClass = Class.forName("etu1759.Model."+file);
            Method[] methods = myClass.getMethods();
            for (Method m : methods) {
                if(m.getAnnotation(etu1759.framework.Url.class)!=null){
                    String an = m.getAnnotation(etu1759.framework.Url.class).key();
    ...
    public String GetMethod(Mapping map){
        try {
            ...
            Class<?> myClass = Class.forName("etu1759.Model."+classN);
            if(myClass.getDeclaredMethod(method).getReturnType().toString().equals("class etu1759.framework.ModelView")){
